...
class IDOFKeypoint(nn.Module):
    stride = None  # strides computed during build
    export = False  # onnx export

    def __init__(self, nc=80, anchors=(), nkpt=5, ch=(), inplace=True, dw_conv_kpt=False, dw_conv_dof=False):  # detection layer
        super(IDOFKeypoint, self).__init__()
        self.nc = nc  # number of classes
        self.nkpt = nkpt
        self.dw_conv_kpt = dw_conv_kpt
        self.dw_conv_dof = dw_conv_dof
        self.no_det=(nc + 5)  # number of outputs per anchor for box and class (cls info(n) + box info(4) + box's conf(1))
        self.no_kpt = 3*self.nkpt ## number of outputs per anchor for keypoints
        self.no_dof = 6 # 6 dof
        self.no = self.no_det+self.no_kpt + self.no_dof
        self.nl = len(anchors)  # number of detection layers
        self.na = len(anchors[0]) // 2  # number of anchors
        self.grid = [torch.zeros(1)] * self.nl  # init grid
        self.flip_test = False
        ...
        if self.dw_conv_dof: #dof head is slightly more complex
            self.m_dof = nn.ModuleList(
                        nn.Sequential(DWConv(x, x, k=3), Conv(x,x),
                                          DWConv(x, x, k=3), Conv(x, x),
                                          DWConv(x, x, k=3), Conv(x,x),
                                          DWConv(x, x, k=3), Conv(x, x),
                                          DWConv(x, x, k=3), Conv(x, x),
                                          DWConv(x, x, k=3), nn.Conv2d(x, self.no_dof * self.na, 1)) for x in ch)
        else: #dof head is a single convolution
            self.m_dof = nn.ModuleList(nn.Conv2d(x, self.no_dof * self.na, 1) for x in ch)
        self.inplace = inplace  # use in-place ops (e.g. slice assignment)
    ...
    def forward(self, x):
        ...
        if self.export:
            for i in range(self.nl):
                if self.nkpt is None or self.nkpt==0:
                    ...
                    x[i] = torch.cat((self.im[i](self.m[i](self.ia[i](x[i]))), self.m_kpt[i](x[i]), self.m_dof[i](x[i])), axis=1)
                #bs, _, ny, nx = x[i].shape  # x(bs,3*(6(box+cls)+15(key-point)+6(dof)),20[grid_x],20[grid_y]) to x(bs,3,20[gird_x],20[grid_y], 6(box+cls)+15(key-point)+6(dof)
                #x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            return x
        ...
        for i in range(self.nl):
            ...
            x[i] = x[i].view(bs, self.na, self.no, ny, nx).permute(0, 1, 3, 4, 2).contiguous()
            x_det = x[i][..., :6]
            x_kpt = x[i][..., 6:6+self.nkpt*3]
            x_dof = x[i][..., 6+self.nkpt*3:]
            ...
            if not self.training:  # inference
                ...
                if self.nkpt == 0:
                    ...
                    y = x_det.sigmoid()
                # if self.inplace == False, it does not run code (y's problem)
                if self.inplace:
                    xy = (y[..., 0:2] * 2. - 0.5 + self.grid[i]) * self.stride[i]  # xy
                    wh = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i].view(1, self.na, 1, 1, 2) # wh
                    if self.nkpt != 0:
                    ...
                    y = torch.cat((xy, wh, y[..., 4:], x_kpt, x_dof), dim = -1)

                else:  # for YOLOv5 on AWS Inferentia https://github.com/ultralytics/yolov5/pull/2953
                    # this is a workaround for the issue that y is not updated in the above code 
                    xy = (y[..., 0:2] * 2. - 0.5 + self.grid[i]) * self.stride[i]  # xy
                    wh = (y[..., 2:4] * 2) ** 2 * self.anchor_grid[i]  # wh
                    if self.nkpt != 0:
                        y[..., 6:] = (y[..., 6:] * 2. - 0.5 + self.grid[i].repeat((1,1,1,1,self.nkpt))) * self.stride[i]  # xy
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        elif m in [Detect, IDetect, IKeypoint, IDOFKeypoint]:
            args.append([ch[x] for x in f])
            if isinstance(args[1], int):  # number of anchors
                args[1] = [list(range(args[1] * 2))] * len(f)
            if 'dw_conv_kpt' in d.keys():
                args_dict = {"dw_conv_kpt" : d['dw_conv_kpt']}
            if 'dw_conv_dof' in d.keys():
                args_dict = {"dw_conv_dof" : d['dw_conv_dof']}
        elif m is ReOrg:
            c2 = ch[f] * 4
        elif m is Contract:
            c2 = ch[f] * args[0] ** 2
